...
ATask = {
    ...
    report: function (C, next) {
        var t = require('tap').test('Api Validator execute the test plan on requests list "' + C.requestYaml + '"');

        lodash.map(C.results, function (R, I) {
            t.ok(C.validates[C.savedFiles[I]] ? true : false, C.savedFiles[I] + ' - ' + R.url);
            t.end();
        });
        next(C);
    },
